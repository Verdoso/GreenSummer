...
public class ClusteredConcurrentIndexedCollection<K extends Comparable<K>, O extends Serializable> extends ConcurrentIndexedCollection<O>
    ...
    public ClusteredConcurrentIndexedCollection(String name, HazelcastInstance hazelcastInstance, SimpleFunction<O, K> primaryKeyFunction,
            ClusteredCollectionEntryListener<O> entryListener) {
        this(name, hazelcastInstance, attribute(name + "_PK", primaryKeyFunction), entryListener);
    }

    public ClusteredConcurrentIndexedCollection(String name, HazelcastInstance hazelcastInstance, SimpleFunction<O, K> attribute) {
    ...
    public boolean retainAll(Collection<?> objectsToRetain) {
        if (this.isLocallyHandled) {
            Set<K> mapToRetain = ((Collection<O>) objectsToRetain).stream().map(this::getKey).collect(Collectors.toSet());
